...
class TestDialogClassifier:
    ...
    def test_classify_continue_actions(self):
        ...
        for text in continue_texts:
            result = self.processor.classify_dialog_action(text)
            assert result == DialogAction.CONTINUE, f"'{text}' should be classified as CONTINUE"
    ...
    def test_classify_approve_actions(self):
        ...
        for text in approve_texts:
            result = self.processor.classify_dialog_action(text)
            assert result == DialogAction.APPROVE, f"'{text}' should be classified as APPROVE"
    
    def test_classify_cancel_approve_actions(self):
        """결재 취소 관련 액션 분류 테스트"""
        cancel_approve_texts = [
            "취소 결재를 진행하시겠습니까",
            "취소 결재"
        ]
        ...
        for text in cancel_approve_texts:
            result = self.processor.classify_dialog_action(text)
            assert result == DialogAction.CANCEL_APPROVE, f"'{text}' should be classified as CANCEL_APPROVE"
    ...
    def test_classify_unknown_actions(self):
        ...
        for text in unknown_texts:
            result = self.processor.classify_dialog_action(text)
            assert result == DialogAction.UNKNOWN, f"'{text}' should be classified as UNKNOWN"
    ...
    def test_pattern_priority(self):
        ...
        text_with_multiple_patterns = "결재문서가 완료되었습니다.\n\n종료하시겠습니까? 다음"
