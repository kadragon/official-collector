...
def temp_supabase_service() -> Generator[SupabaseService, None, None]:
    ...
    try:
        ...
        service.client.table('task_card_mappings').delete().like('title', 'pytest_%').execute()
        service.client.table('reception_mappings').delete().like('title', 'pytest_%').execute()
        yield service
...
class TestEnvironmentConfig:
    ...
    def test_supabase_config_format(self):
        ...
        assert len(supabase_key) > 20, \
            "SUPABASE_KEY should be a proper JWT token"
...
class TestOpenAIIntegration:
    ...
    def test_openai_connection(self):
        ...
        try:
            ...
            assert len(result.embedding) == 1536, "Should return 1536-dimension embedding"
            assert all(isinstance(x, (int, float)) for x in result.embedding), "Embedding should contain numbers"
        except Exception as e:
    ...
    def test_embedding_consistency(self):
        ...
        try:
            ...
            assert result1.embedding == result2.embedding, "Same input should produce same embedding"
        except Exception as e:
            if "API" in str(e) or "rate" in str(e).lower():
                pytest.skip(f"OpenAI API issue: {e}")
            raise
...
class TestSupabaseService:
    ...
    def test_reception_operations(self, temp_supabase_service):
        ...
        # Test upsert
        result = service.upsert_reception_embedding(test_title, test_handler, test_share)
        ...
        assert handler == test_handler, f"Expected handler '{test_handler}', got '{handler}'"
    ...
    def test_document_count(self, temp_supabase_service):
        ...
        assert reception_count >= 1, f"Should have at least 1 reception, got {reception_count}"
    
    def test_parametrized_card_operations(self, temp_supabase_service, sample_card_data):
    ...
    def test_parametrized_reception_operations(self, temp_supabase_service, sample_reception_data):
    ...
    def test_edge_cases(self, temp_supabase_service, sample_test_dataset):
        ...
        try:
            result = service.upsert_card_embedding(
                f"pytest_{edge_cases['special_chars']}", 
                "pytest_special_card"
            )
        ...
        try:
            result = service.upsert_card_embedding(
                f"pytest_{edge_cases['mixed_lang']}", 
                "pytest_mixed_card"
            )
    ...
    def test_error_recovery(self, temp_supabase_service):
        ...
        assert handler is None and share is None, "Should return (None, None) for non-existent reception"
...
class TestMockIntegration:
    ...
    def test_embedding_generation_mock(self):
        ...
        with patch('services.supabase_service.create_client') as mock_client, \
             patch('services.supabase_service.OpenAIEmbeddingService') as mock_embedding_class:
