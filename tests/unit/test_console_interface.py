...
class TestDisplayWidth:
    ...
    @pytest.mark.parametrize("text,expected_width", [
        ("hello", 5),          # 영문
        ("안녕하세요", 10),        # 한글 5자 = 10칸
...
class TestConsoleInterface:
    ...
    @patch('builtins.input')
    def test_select_approval_and_share_single_approval(self, mock_input, mock_clear, mock_subprocess):
        ...
        approval, share = self.console.select_approval_and_share(approval_list, share_list)
    ...
    @patch('ui.console_interface.clear_screen')
    @patch('builtins.input')
    def test_select_approval_and_share_multiple_options(self, mock_input, mock_clear, mock_subprocess):
    ...
    def test_confirm_recommendation_yes(self, mock_input, mock_clear, mock_subprocess):
        ...
        result = self.console.confirm_recommendation(
            "테스트 문서", 
            "추천 과제카드"
        )
    ...
    @patch('ui.console_interface.clear_screen')
    @patch('builtins.input')
    def test_choose_from_predefined_list_skip(self, mock_input, mock_clear, mock_subprocess):
...
class TestDeletionInterface:
    ...
    @patch('ui.console_interface.clear_screen')
    @patch('builtins.input')
    def test_get_title_for_deletion_confirm(self, mock_input, mock_clear, mock_subprocess):
    ...
    @patch('ui.console_interface.clear_screen')
    @patch('builtins.input')
    def test_get_title_for_deletion_cancel(self, mock_input, mock_clear, mock_subprocess):
    ...
    @patch('ui.console_interface.clear_screen')
    @patch('builtins.input')
    def test_confirm_bulk_deletion_confirm(self, mock_input, mock_clear, mock_subprocess):
    ...
    @patch('ui.console_interface.clear_screen')
    @patch('builtins.input')
    def test_confirm_bulk_deletion_cancel(self, mock_input, mock_clear, mock_subprocess):
