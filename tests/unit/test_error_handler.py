...
class TestSetupLogger:
    ...
    def test_logger_handlers_attached(self):
        ...
        assert has_expected_handler, f"Expected handler types {expected_handlers}, got {handler_types}"
    ...
    def test_logger_formatter_configuration(self):
        ...
        for handler in logger.handlers:
            ...
            format_string = formatter._fmt if hasattr(formatter, '_fmt') else str(formatter)
            
            # Should include timestamp, level, and message
            useful_elements = ['%(asctime)s', '%(levelname)s', '%(message)s', '%(name)s']
            has_useful_info = any(element in format_string for element in useful_elements)
            assert has_useful_info, f"Formatter should include useful info: {format_string}"
...
class TestLoggerConfiguration:
    ...
    def test_console_output_configuration(self):
        ...
        has_console = len(console_handlers) > 0 or any(
            ...
            for p in [logger.parent] if p
        )
        assert has_console or len(logger.handlers) == 0  # Empty handlers ok if parent handles
