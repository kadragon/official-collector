...
This runbook provides operational guidance for monitoring, troubleshooting, and maintaining the Official Collector system with Supabase + OpenAI integration.

**Last Updated**: 2025-11-13
**System Version**: Phase 6 (Deployment Ready)
...
4. [API Quota Management](#api-quota-management)
...
The system tracks health metrics for each service (OpenAI, Supabase):
...
**Trigger Conditions**:
- Response time > 5000ms (default threshold)
...
Audit logs are stored in **JSON Lines format** at:
...
1. **Data Modifications**
   - Reception document upserts
   - Task card upserts
   - Data deletions (especially `clear_all_data`)

2. **API Calls**
   - OpenAI embedding generation
   - Supabase vector searches
   - Database queries
...
**Resolution**:
1. Lower similarity threshold (default: 0.3)
   ```bash
   export VECTOR_SIMILARITY_THRESHOLD=0.2
   ```
2. Verify document count:
   ```python
   supabase.get_document_count("task_card")
   ```
...
**Symptoms**:
- "Quota check failed" errors
- API calls blocked
- EXCEEDED status in logs
...
# Should call init_audit_logger() at startup
```
...
- **v1.0** (2025-11-13): Initial runbook for Phase 6 deployment readiness
  - Added audit logging
