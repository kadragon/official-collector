...
class UnifiedDebugManager:
    ...
    def __init__(self, app_title_patterns: Optional[List[str]] = None, interactive: bool = True):
        """
    ...
    def _setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler(
                    log_dir / f'debug_session_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
    ...
    def connect_to_app(self) -> bool:
        ...
        for pattern in self.app_title_patterns:
            try:
                self.app = Application(backend="win32").connect(title_re=f".*{pattern}.*")
                self.dlg = self.app.top_window()
                logger.info(f"Successfully connected to application with pattern: {pattern}")
                return True
            except Exception as e:
                logger.debug(f"Failed to connect with pattern '{pattern}': {e}")
                continue

        logger.error(f"Failed to connect to any application with patterns: {self.app_title_patterns}")
        return False
    ...
    def analyze_current_window(self, app_patterns: Optional[List[str]] = None) -> Dict[str, Any]:
        """
    ...
    def monitor_dialogs(self, duration: float = 30.0, interval: float = 1.0) -> Union[None, List[Dict[str, Any]]]:
        """
        ...
        logger.info(f"Starting dialog monitoring for {duration} seconds (interval: {interval}s)")
    ...
    def run_interactive_session(self):
        ...
        while True:
            ...
            if choice == '1':
                ...
                duration = float(input("Enter monitoring duration (seconds, default 30): ") or "30")
                interval = float(input("Enter monitoring interval (seconds, default 1): ") or "1")
...
def create_debug_manager(interactive: bool = True, app_patterns: Optional[List[str]] = None) -> UnifiedDebugManager:
    """
