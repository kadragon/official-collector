...
class ReceptionService:
    ...
    '접수' 공문을 처리하는 클래스.
    """
    def __init__(self, chroma_service: ChromaService, dialog_handler: DialogHandler, approval_name_list: List[str], share_name_list: List[str]):
    ...
    def handle_reception(self, title: str) -> Tuple[Optional[str], Optional[Any]]:
        ...
        # 1. 정확한 제목 매칭을 먼저 시도
        approval, shared = self.chroma_service.retrieve_reception_by_title(processed_title)
        if approval:
        ...
        if processed_title:
            recommendations = self.chroma_service.recommend_reception(processed_title, count=3)
            ...
            if recommendations:
                ...
                if status == SelectionResult.SELECTED:
                    ...
                    self.chroma_service.upsert_reception_embedding(processed_title, approval, shared)
