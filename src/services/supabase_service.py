...
class SupabaseService:
    ...
    def get_reception_document(self, document_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            result = (
                self.client.table("reception_documents")
                .select("*")
                .eq("id", document_id)
                .execute()
            )
    ...
    def get_task_card(self, task_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            result = (
                self.client.table("task_cards").select("*").eq("id", task_id).execute()
            )
            return result.data[0] if result.data else None
        except Exception as e:
    ...
    def list_task_cards(
        self, category: str = None, status: str = None, limit: int = 100
    ) -> List[Dict[str, Any]]:
        """업무카드 목록 조회"""
        try:
    ...
    def batch_update_documents(
        self, document_type: str, updates: List[Dict[str, Any]]
    ) -> bool:
        """문서 배치 업데이트"""
        try:
            table_name = (
                "reception_documents"
                if document_type == "reception_document"
                else "task_cards"
            )
            ...
            for update in updates:
                ...
                self.client.table(table_name).update(update).eq(
                    "id", document_id
                ).execute()
    ...
    def upsert_reception_embedding(self, title: str, handler: str, share_target: str):
        ...
        try:
            ...
            if existing.data:
                ...
                logger.info(
                    f"접수 문서 매핑 업데이트: {title} -> {handler}/{share_target}"
                )
            else:
                # 삽입
