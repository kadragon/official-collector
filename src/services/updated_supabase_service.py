...
class SupabaseService:
    ...
    def list_task_card_mappings(
        self, task_title: str = None, limit: int = 100
    ) -> List[Dict[str, Any]]:
        """업무카드 매핑 목록 조회"""
        try:
    ...
    def list_reception_mappings(
        self, handler: str = None, limit: int = 100
    ) -> List[Dict[str, Any]]:
        """접수 매핑 목록 조회"""
        try:
    ...
    def batch_insert_task_mappings(self, mappings_data: List[Dict[str, Any]]) -> bool:
        ...
        try:
            ...
            result = (
                self.client.table("task_card_mappings").insert(mappings_data).execute()
            )
    ...
    def batch_insert_reception_mappings(
        ...
        try:
            ...
            result = (
                self.client.table("reception_mappings").insert(mappings_data).execute()
            )
    ...
    # 검색 및 추천 메서드
    def recommend_task_card(
        self, title: str, embedding_service, top_k: int = 5
    ) -> List[Dict[str, Any]]:
        """공문 제목으로 업무카드 추천"""
        try:
            # 제목 임베딩 생성
            embedding_result = embedding_service.create_embedding(
                title, f"query_{int(datetime.now().timestamp())}"
            )
            ...
            similar_mappings = self.find_similar_task_cards(
                embedding_result.embedding, limit=top_k
            )
    ...
    def recommend_handler(
        self, title: str, embedding_service, top_k: int = 3
    ) -> List[Dict[str, Any]]:
        """공문 제목으로 담당자 추천"""
        try:
            # 제목 임베딩 생성
