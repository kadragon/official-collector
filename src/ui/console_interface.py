...
def draw_header(title: str, width=60):
    ...
    print(f"{Colors.BOLD}{Colors.CYAN}│{' ' * padding}{Colors.WHITE}{title}{Colors.CYAN}{' ' * remaining}│{Colors.RESET}")
    print(f"{Colors.BOLD}{Colors.CYAN}{'╰' + '─' * (width - 2) + '╯'}{Colors.RESET}")
    print()
...
def print_selection_menu(title: str, items: List[str], allow_skip=False, skip_text="목록에 없음"):
    """선택 메뉴를 출력합니다."""
    print()
    print(f"{Colors.BOLD}{Colors.WHITE}┌─ {title} {'─' * (56 - get_display_width(title))}┐{Colors.RESET}")
...
def get_user_choice_from_list(options: List[str], allow_skip: bool = False) -> Tuple[SelectionResult, Optional[str]]:
...
def get_user_choice_from_recommendations(title: str, recommendations: List[str]) -> Tuple[SelectionResult, Optional[str]]:
    ...
    print_selection_menu("추천 목록", recommendations,
                         allow_skip=True, skip_text="추천 없음 (다음 단계로 이동)")
    ...
    while True:
        try:
            ...
            logger.warning(f"유효하지 않은 입력 (추천에서): {selection if 'selection' in locals() else '알 수 없음'}")
            print("유효하지 않은 입력입니다. 번호를 입력해주세요.")
...
class ConsoleInterface:
    ...
    def choose_from_predefined_list(self, title: str, card_list: List[str]) -> Tuple[SelectionResult, Optional[str]]:
        ...
        print_selection_menu("목록에서 선택", card_list,
                             allow_skip=True, skip_text="목록에 없음")
    ...
    def confirm_bulk_deletion(self) -> bool:
        ...
        print(f"{Colors.RED}║{Colors.BOLD}{Colors.WHITE}    ⚠️  일괄 삭제 경고    {Colors.RESET}{Colors.RED}║{Colors.RESET}")
        print(
        ...
        return confirm_choice("정말로 모든 데이터를 삭제하시겠습니까?", default_yes=False)
