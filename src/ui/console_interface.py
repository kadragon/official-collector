...
class ConsoleInterface:
    ...
    def confirm_recommendation(self, title: str, recommended_task_title: str) -> bool:
        ...
        print()
        print(f"{Colors.BOLD}{Colors.CYAN}{'─' * 60}{Colors.RESET}")
        print(f"{Colors.BOLD}{Colors.CYAN}  추천 확인{Colors.RESET}")
        print(f"{Colors.BOLD}{Colors.CYAN}{'─' * 60}{Colors.RESET}")
        print()
        
        print(f"  {Colors.DIM}공문 제목:{Colors.RESET} {title}")
        print(f"  {Colors.GREEN}추천 항목:{Colors.RESET} {Colors.BOLD}{recommended_task_title}{Colors.RESET}")
        print()
    ...
    def choose_from_predefined_list(self, title: str, card_list: List[str]) -> Tuple[SelectionResult, Optional[str]]:
        ...
        print_selection_menu("목록에서 선택", card_list, allow_skip=True, skip_text="목록에 없음")
    ...
    def confirm_bulk_deletion(self) -> bool:
        ...
        print(f"\n{Colors.RED}=== 일괄 삭제 경고 ==={Colors.RESET}")
